...
class Thread:
    def __init__(self, address):
        if address != NULL_PTR:
            ...
            self.continuation = getPointerAt(
            ...
            self.uContextData = getPointerAt(
                address + ThreadOffsets.CONTEXT_USER_DATA_PTR.value)
            self.kernel_stack_ptr = getPointerAt(
                address + ThreadOffsets.KSTACK_PTR.value)
            self.voucher_ptr = getPointerAt(
    ...
    def getKernelNextPc(self):
        ...
        next_to_thread_invoke = getPointerAt(
    ...
    def printTheadInfoShort(self, max_length_proc, max_length_cont, max_length_pc):
        ...
        res_str += f" [{self.task_object.bsdinfo_object.bsd_pid}] | \
            {self.task_object.bsdinfo_object.bsd_name:<{max_length_proc}} |"
        res_str += f" {self.tid} | {hex(self.address)} |"
        res_str += f' {"N/A":^{max_length_cont}} |' if self.continuation == NULL_PTR \
            else f" {getSymbol(hex(self.continuation)):^{max_length_cont}} |"
        res_str += f' {"N/A":^{max_length_pc}} |' if self.next_pc == NULL_PTR \
            else f" {getSymbol(hex(self.next_pc)):^{max_length_pc}} |"
    ...
    def printThreadInfoLong(self):
        ...
        if self.task_object:
            res_str += f"thread->task->bsd_info                 \
                {printPtrAsString(self.task_object.bsd_info_ptr)}\n"
        if self.task_object.bsd_info_ptr:
            ...
            res_str += f"thread->task->bsd_info->bsd_pid       \
                {printPtrAsString(self.task_object.bsdinfo_object.bsd_pid)}\n"
        ...
        res_str += f"thread->kstackptr                      \
            {printPtrAsString(self.kernel_stack_ptr)}\n"
...
class BsdInfo:
    def __init__(self, address):
        if address != NULL_PTR:
            self.bsd_pid = getIntAt(
                address + BSDInfoOffsets.PID_IN_BSD_INFO.value)
            self.bsd_name = getStringAt(
...
class IPCObject:
    def __init__(self, address):
        if address != NULL_PTR:
            ...
            self.io_references = getIntAt(
                address + IPCObjectOffsets.IO_REFS.value)
            self.io_lock_data_1 = getPointerAt(
                address + IPCObjectOffsets.IO_LOCK_DATA.value)
            self.io_lock_data_2 = getPointerAt(
...
class IPCPort:
    def __init__(self, address):
        if address != NULL_PTR:
            ...
            self.ip_context = getPointerAt(
                address + IPCPortOffsets.IP_CTXT.value)
            self.ip_sprequests = (
                getIntAt(address + IPCPortOffsets.IP_SPREQ.value) & (1 << 0))
            self.ip_spimportant = (
                getIntAt(address + IPCPortOffsets.IP_SPREQ.value) & (1 << 1))
            self.ip_impdonation = (
                getIntAt(address + IPCPortOffsets.IP_SPREQ.value) & (1 << 2))
            self.ip_tempowner = (
                getIntAt(address + IPCPortOffsets.IP_SPREQ.value) & (1 << 3))
            self.ip_guarded = (
                getIntAt(address + IPCPortOffsets.IP_SPREQ.value) & (1 << 4))
            self.ip_strict_guard = (
                getIntAt(address + IPCPortOffsets.IP_SPREQ.value) & (1 << 5))
            self.ip_specialreply = (
                getIntAt(address + IPCPortOffsets.IP_SPREQ.value) & (1 << 6))
            self.ip_sync_link_state = (getIntAt(
                address + IPCPortOffsets.IP_SPREQ.value) & (0x000001ff))
            self.ip_impcount = (getPointerAt(
                address + IPCPortOffsets.IP_SPREQ.value) & (0xfffffe00))
            self.ip_mscount = getPointerAt(
                address + IPCPortOffsets.IP_MSCNT.value)
            self.ip_srights = getPointerAt(
                address + IPCPortOffsets.IP_SRIGHTS.value)
            self.ip_sorights = getPointerAt(
...
class Task:
    ...
    def printTaskInfoLong(self):
        ...
        res_str += f"task->ipc_space->is_table = \
            {printPtrAsString(self.ipc_space_object.is_table)}\n"
        return res_str
...
class ThreadVoucher:
    def __init__(self, address):
        if address != NULL_PTR:
            ...
            self.iv_tabl = getPointerAt(address + 0x30)
...
class ThreadsIterator:
    ...
    def __next__(self):
        if self.next_thread_ptr != self.stop_contition:
            ...
            if self.type == ThrdItrType.GLOBAL:
                self.next_thread_ptr = getPointerAt(
